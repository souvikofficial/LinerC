...
export let aggregatesWorker: Worker | undefined;

(async () => {
    try {
        await waitForReady(2000);
    } catch (e) {
        console.warn('Skipping aggregates worker start: Redis not ready');
        return;
    }

    try {
        aggregatesWorker = new Worker('rollup-aggregates', async (job) => {
        ...
        console.log('Started aggregates worker');
    } catch (err) {
        console.warn('Failed to start aggregates worker', err);
    }
})();
